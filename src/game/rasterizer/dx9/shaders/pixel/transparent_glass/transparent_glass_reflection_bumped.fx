// Bump map texture
// Force it to use register s1 (when compiled uses s0) as the bump map texture is in here for CE
SamplerState TexSampler0 : register(s1);
Texture2D Texture0 : register(t1);

// Specular reflection cube map
// Force it to use register s2 (when compiled uses s3) as here is where Halo CE stores the cube map
