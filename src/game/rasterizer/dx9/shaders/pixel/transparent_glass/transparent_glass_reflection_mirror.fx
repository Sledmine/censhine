...
half4 main(PS_INPUT i) : SV_TARGET
{
	...
	E = lerp(E1, E2, params.y);
	N = lerp(N1, N2, params.y);
	
	half3 reflection_color = Texture3.Sample(TexS3, Tex3.xy / Tex3.w); // texture3: mirror
	half3 specular_color = pow(reflection_color, 8);
	
	half diffuse_reflection = pow(dot(N, E), 2);
	
	// params.x group intensity
   	half3 specular_mask = lerp(Diff.a * params.x, t0.rgb, params.z); // params.z 1 -> bump_map_is_specular_mask 
